...
public class TrajectoryPredictor : MonoBehaviour
{
    ...
    public int distanceBetweenBalls = 2;
    ...
    public void SpawnTrajectoryBalls(ProjectileProperties projectile)
    {
        ...
        GameObject ballWithMaxVelocity = null;
        GameObject ballWithMaxHeight = null;
            ...
            if (i % distanceBetweenBalls == 0)
            {
                ...
                if (Physics.Raycast(ray, out RaycastHit hit, Mathf.Infinity))
                {
                    if (hit.distance > 0)
                    {
                        ...
                        if (currentVelocity == maxVelocity)
                        {
                            ballWithMaxVelocity = ball;
                        }

                        if (nextPosition.y == highestPoint)
                        {
                            ballWithMaxHeight = ball;
                        }
        ...
        if (ballWithMaxVelocity != null)
        {
            ballWithMaxVelocity.gameObject.transform.localScale = ballWithMaxVelocity.gameObject.transform.localScale * 1.3f;
            MeshRenderer renderer = ballWithMaxVelocity.GetComponent<MeshRenderer>();
            if (renderer != null)
            {
        ...
        if (ballWithMaxHeight != null)
        {
            ballWithMaxHeight.gameObject.transform.localScale = ballWithMaxHeight.gameObject.transform.localScale * 1.3f;
            MeshRenderer renderer = ballWithMaxHeight.GetComponent<MeshRenderer>();
            if (renderer != null)
            {
    ...
    public void ChangeBallDistance(TMP_InputField input)
    {
        if (string.IsNullOrEmpty(input.text))
            return;

        if (int.TryParse(input.text, out int newMaxDistance))
        {
            if (newMaxDistance <= 1f)
            {
                distanceBetweenBalls = 1;
                input.text = distanceBetweenBalls.ToString();
            }
            else if (newMaxDistance >= 3)
            {
                distanceBetweenBalls = 3;
                input.text = distanceBetweenBalls.ToString();
            }
            else
            {
                maxPoints = newMaxDistance;
            }
        }
    }
